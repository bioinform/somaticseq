...
parser.add_argument(
    ...
    "--label1",
    type=str,
    help="Label 1 for TrueVariants_or_False",
    required=False,
    default="True Positives",
)
parser.add_argument(
    "-label0",
    "--label0",
    type=str,
    help="Label 0 for TrueVariants_or_False",
    required=False,
...
def plot_2hist(false_values, true_values, hist_bins=None, labels=None, hist_title=None):
    ...
    if not hist_bins:
        try:
            hist_bins = (
                min(min(false_values), min(true_values)),
                max(max(false_values), max(true_values)),
                50,
            )
        except ValueError:
            hist_bins = (0, 1, 2)
