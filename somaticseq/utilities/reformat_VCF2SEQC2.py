...
parser.add_argument(
    "-callers",
    "--callers-classification-string",
    type=str,
    help="MVJSD or whatever",
    required=True,
)
parser.add_argument(
    "-tumor",
...
parser.add_argument(
    ...
    "--somaticseq-trained",
    action="store_true",
    help="If true, will use the QUAL as SomaticSeq score. Otherwise, SCORE will be .",
    required=False,
    default=False,
)
...
with genome.open_textfile(vcf_in_fn) as vcfin, open(vcf_out_fn, "w") as vcfout:
    ...
    while line_in:
        ...
        line_out = "\t".join(
            (
                vcf_line_in.chromosome,
                str(vcf_line_in.position),
                vcf_line_in.identifier,
                vcf_line_in.refbase,
                vcf_line_in.altbase,
                vcf_line_in.qual,
                vcf_line_in.filters,
