...
* You can also do it in parallel using `$somaticseq/somaticseq/run_parallel.py` script, with identical input options except for `--threads` placed before the `paired` option to indicate the number of threads. The `run_parallel.py` is simply a script to create multiple sub-BED files, and then invokes `somaticseq/run_somaticseq.py` on each of those sub-BED files in parallel.
