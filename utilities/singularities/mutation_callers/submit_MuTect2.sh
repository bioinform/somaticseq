...
then
    ...
    echo "tumor_name=\`singularity exec --bind /:/mnt docker://lethalfang/samtools:0.1.19 samtools view -H /mnt/${tumor_bam} | egrep -w '^@RG' | grep -Po 'SM:[^\t$]+' | sed 's/SM://' | uniq | sed -e 's/[[:space:]]*$//'\`" >> $out_script
fi
...
then
    ...
    echo "normal_name=\`singularity exec --bind /:/mnt docker://lethalfang/samtools:0.1.19 samtools view -H /mnt/${normal_bam} | egrep -w '^@RG' | grep -Po 'SM:[^\t$]+' | sed 's/SM://' | uniq | sed -e 's/[[:space:]]*$//'\`" >> $out_script
    echo "" >> $out_script
fi

echo "" >> $out_script

echo "singularity exec --bind /:/mnt docker://broadinstitute/gatk:4.beta.5 \\" >> $out_script
