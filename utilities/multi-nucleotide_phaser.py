...
import argparse, pysam, itertools, os, sys
...
pysam.FastaFile(ref_fa) as ref_fa:
    ...
    while my_line:
        ...
        while (my_coordinates[-1][0] == my_vcf.chromosome) and (my_vcf.position - my_coordinates[-1][1] <= threshold):
            ...
            elif (my_coordinates[-1][0] == my_vcf.chromosome) and (my_vcf.position - my_coordinates[-1][1] <= threshold):
        ...
        if 6 > len(my_coordinates) > 1:
            ...
            for read_i in reads:
                if not (read_i.is_unmapped or read_i.is_duplicate):
                    ...
                    for coordinate_i in my_coordinates:
                        ...
                        code_i, ith_base, base_call_i, indel_length_i, flanking_indel_i = position_of_aligned_read(read_i, coordinate_i[1]-1 )
            ...
            mnp_line = '{CHR}\t{POS}\t.\t{REF}\t{ALT}\t.\t{FILTER}\t{INFO}\t{FORMAT}\t{SAMPLE}'.format(CHR=my_coordinates[0][0], POS=my_coordinates[0][1], REF=ref_string, ALT=','.join(mnp_calls), FILTER=filters_i, INFO=info_line, FORMAT='GT', SAMPLE='0/1')
