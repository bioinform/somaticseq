...
DEFAULT_PARAMS = {'strelka2_image'           : 'lethalfang/strelka:2.9.5',
                  'MEM'                      : '4G',
                  'threads'                  : 1,
...
def tumor_normal(input_parameters=DEFAULT_PARAMS, tech='docker' ):
    ...
    container_line, fileDict = container.container_params( input_parameters['strelka2_image'], tech=tech, files=all_paths, extra_args=input_parameters['extra_docker_options'] )
    ...
    with open(outfile, 'w') as out:
        ...
        tabix_line, tabixDict = container.container_params( 'lethalfang/tabix:1.7', tech, all_paths )
        ...
        out.write( '{DOCKER_LINE} bash -c "cat {SELECTOR} | bgzip > {OUTDIR}/{BEDGZ}\"\n'.format(DOCKER_LINE=tabix_line, SELECTOR=tabix_selector, OUTDIR=tabix_outdir, BEDGZ=bed_gz ) )
        out.write( '{DOCKER_LINE} tabix -f {OUTDIR}/{BEDGZ}\n\n'.format(DOCKER_LINE=tabix_line, OUTDIR=tabix_outdir, BEDGZ=bed_gz ) )
        ...
        out.write(f'{container_line} \\\n' )
        out.write( '{}/{}/runWorkflow.py -m local -j 1 {}\n'.format(mounted_outdir, input_parameters['outdir_name'], input_parameters['strelka_run_arguments']) )
