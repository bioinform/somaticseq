...
echo "docker run --rm -v /:/mnt -u $UID --memory 16g lethalfang/jointsnvmix2:0.7.5 \\" >> $jsm_script
...
echo "docker run --rm -v /:/mnt --memory 16g lethalfang/jointsnvmix2:0.7.5 \\"  >> $jsm_script
...
echo "| docker run --rm -v /:/mnt -u $UID --memory 16g -i lethalfang/jointsnvmix2:0.7.5 \\" >> $jsm_script
...
then
    ...
    echo "do" >> $jsm_script
    echo "    docker run --rm -v /:/mnt -u $UID --memory 6g lethalfang/somaticseq:base-1.0 bedtools intersect -a /mnt/${outdir}/${outvcf} -b /mnt/${outdir}/\${i}/\${i}.bed -header | uniq >  ${outdir}/\${i}/${outvcf}" >> $jsm_script
