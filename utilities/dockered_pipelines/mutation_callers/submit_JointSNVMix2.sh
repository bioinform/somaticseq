...
then
    ...
    echo "do" >> $out_script
    echo "    docker run --rm -v /:/mnt -u $UID lethalfang/bedtools:2.26.0 bash -c \"bedtools intersect -a /mnt/${outdir}/${outvcf} -b /mnt/${outdir}/\${i}/\${i}.bed -header | uniq > /mnt/${outdir}/\${i}/${outvcf}\"" >> $out_script
