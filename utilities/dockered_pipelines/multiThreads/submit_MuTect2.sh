...
then
    ...
    echo "tumor_name=\`docker run -v /:/mnt -u $UID -i lethalfang/samtools:0.1.19 samtools view -H /mnt/${tumor_bam} | egrep -w '^@RG' | egrep -o 'SM:.+' | sed 's/SM://' | uniq | sed -e 's/[[:space:]]*$//'\`" >> $mutect2_script
fi
...
then
    ...
    echo "normal_name=\`docker run -v /:/mnt -u $UID -i lethalfang/samtools:0.1.19 samtools view -H /mnt/${normal_bam} | egrep -w '^@RG' | egrep -o 'SM:.+' | sed 's/SM://' | uniq | sed -e 's/[[:space:]]*$//'\`" >> $mutect2_script
    echo "" >> $mutect2_script
fi
