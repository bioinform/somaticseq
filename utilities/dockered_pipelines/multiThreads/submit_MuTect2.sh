...
then
    ...
    echo "tumor_name=\`docker run -v /:/mnt -u $UID --rm -i lethalfang/samtools:0.1.19 samtools view -H /mnt/${tumor_bam} | egrep -w '^@RG' | grep -Po 'SM:[^\t$]+' | sed 's/SM://' | uniq | sed -e 's/[[:space:]]*$//'\`" >> $mutect2_script
fi
...
then
    ...
    echo "normal_name=\`docker run -v /:/mnt -u $UID --rm -i lethalfang/samtools:0.1.19 samtools view -H /mnt/${normal_bam} | egrep -w '^@RG' | grep -Po 'SM:[^\t$]+' | sed 's/SM://' | uniq | sed -e 's/[[:space:]]*$//'\`" >> $mutect2_script
    echo "" >> $mutect2_script
fi

echo "" >> $mutect2_script

echo "docker run -v /:/mnt -u $UID --rm -i broadinstitute/gatk:4.beta.3 \\" >> $mutect2_script
