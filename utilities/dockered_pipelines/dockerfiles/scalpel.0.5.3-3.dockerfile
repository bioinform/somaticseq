...
RUN cd /opt && wget https://downloads.sourceforge.net/project/scalpel/scalpel-0.5.3.tar.gz && tar -xvf scalpel-0.5.3.tar.gz && ln -s scalpel-0.5.3 scalpel && cd scalpel-0.5.3 && make
RUN cd /opt/scalpel-0.5.3/samtools-1.1 && make && make install
