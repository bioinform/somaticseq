...
# -truth is also optional, but it is needed to annotate ground truth information, and is thus required for model training.
# -dedup will ignore reads that are marked duplicates.
SSeq_merged.vcf2tsv.py -ref genome.GRCh37.fa -myvcf somatic.snp.processed.vcf -truth Ground.truth.snp.vcf -mutect MuTect/variants.snp.vcf.gz -varscan VarScan2/variants.snp.vcf -jsm JSM2/variants.vcf -sniper SomaticSniper/variants.vcf -vardict VarDict/snp.variants.vcf -dedup -tbam tumor.bam -nbam normal.bam -outfile Ensemble.sSNV.tsv
\end{lstlisting}
...
All the options for SSeq\_merged.vcf2tsv.py can be displayed by running SSeq\_merged.vcf2tsv.py --help. We're listing them here:
...
\begin{lstlisting}
  ...
  -mutect      VCF file from either MuTect or Indelocator
  ...
  -minMQ       Minimum mapping quality for reads to be considered
  -minBQ       Minimum base quality for reads to be considered
  -mincaller   Minimum number of callers for a call to be considered (Use 0.5 to consider some LowQual calls. Default = 0).
...
Note: Do not worry if Python throws some warning like this. 
...
# Use -tools to list ONLY the individual tools used to have appropriately annotated VCF files. Accepted tools are CGA (for MuTect/Indelocator), VarScan2, JointSNVMix2, SomaticSniper, VarDict, MuSE, LoFreq. and/or Scalpel.
...
\begin{itemize}
  ...
  \item
  Properly handle cases with multiple ALT calls in the same position. The VCF files can either have more than one call in the ALT column (i.e., A,G), or have multiple lines corresponding to the same position (one line for each call).
  
  \item
  Deprecated HaplotypeCaller dependencies.
  
  \item
  Improved the Wrapper script to remove SnpSift/SnpEff dependencies. Those information can be directly obtained during the SSeq\_merged.vcf2tsv.py step. Also removed some additional legacy steps that were useless (i.e., score\_Somatic.Variants.py), and added a step to check the correctness of the input. The v2.1 and 2.1.1 had some typos in the wrapper script, but no change in the core programs.
