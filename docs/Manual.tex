...
\begin{enumerate}
	...
	\item Version 3.7.2
        ...
        \begin{itemize}
            ...
            \item Most robustly check sorting order when VCF files are being read. Raise Exception when they are not sorted according to the reference (implemented in the \textit{catchup\_multilines} function in \textit{somaticseq/genomicFileHandler/genomic\_file\_handlers.py}).
        \end{itemize}
	...
	\item Version 3.7.3
        ...
        \begin{itemize}
            \item Allow xgboost extra hyperparameters be passed into \textit{somaticseq\_parallel.py} like \textit{somaticseq\_parallel.py \texttt{--}somaticseq-train \texttt{--}extra-hyperparameters scale\_pos\_weight:0.1 seed:100}. They could always be passed into \textit{somatic\_xgboost.py}. Beware, however, multi-argument options like \textit{\texttt{--}extra-hyperparameters} and \textit{\texttt{--}features-excluded} cannot be placed immediately before \textit{paired}, because otherwise it'll try to include \textit{paired} as an argument instead of invoking \textit{paired} mode.
