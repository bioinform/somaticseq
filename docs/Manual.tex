...
\begin{enumerate}
	...
	\item Version 3.7.0
        ...
        \begin{itemize}
            \item Allow input of any arbitrary VCF file in addition to the callers we have already incorporated (listed in Introduction), e.g., \textit{--arbitrary-snvs extra\_snv\_X.vcf extra\_snv\_Y.vcf --arbitrary-indels extra\_indel\_X.vcf extra\_indel\_Y.vcf}. Must seperate the SNVs and indels into separate VCF files before using them as input to SomaticSeq. Calls labeled REJECT will not be counted. Calls without filter label will be counted as a bona fide call for that particular VCF file. 
