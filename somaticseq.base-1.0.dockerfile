...
RUN apt-get update && apt-get install -y r-base python3 python3-pip bedtools git wget default-jre && apt-get clean && pip3 install cython regex pysam numpy scipy && R -e "install.packages('ada', repos = 'http://cran.rstudio.com/')"
