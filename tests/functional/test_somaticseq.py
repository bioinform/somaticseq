...
def test_paired_somaticseq_cli(
    ...
    consensus_snv_vcf = os.path.join(outdir, "Consensus.sSNV.vcf")
    consensus_indel_vcf = os.path.join(outdir, "Consensus.sINDEL.vcf")
    signal = subprocess.call(command_line, shell=True)
    assert signal == 0
    assert os.path.exists(consensus_snv_vcf)
    assert os.path.exists(consensus_indel_vcf)
    assert os.path.exists(os.path.join(outdir, "Ensemble.sSNV.tsv"))
    assert os.path.exists(os.path.join(outdir, "Ensemble.sINDEL.tsv"))
    assert os.path.exists(
        os.path.join(outdir, f"Ensemble.sSNV.tsv.xgb.v{__version__}.classifier")
    )
    assert os.path.exists(
        os.path.join(outdir, f"Ensemble.sINDEL.tsv.xgb.v{__version__}.classifier")
    )
